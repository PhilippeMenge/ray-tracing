...
class Triangulo(Objeto):
    ...
    """
    def get_intersecao(self, ray: Ray) -> None | float:
        aresta_ab = self.points[1] - self.points[0] # edge1
        aresta_ac = self.points[2] - self.points[0] # edge2
        aresta_bc = self.points[2] - self.points[1] # edge3
        ...
        numerador = normal.produto_escalar(self.points[0] - ray.origem)
        ...
        area_acp = (((ponto_intersecao - self.points[0]).produto_vetorial(aresta_ac)).norma())/2
        u = area_acp/area_total
        
        area_abp = (((ponto_intersecao - self.points[0]).produto_vetorial(aresta_ab)).norma())/2
        v = area_abp/area_total

        area_bcp = (((ponto_intersecao - self.points[1]).produto_vetorial(aresta_bc)).norma())/2
